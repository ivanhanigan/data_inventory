...
#+begin_src R :session *R* :tangle no :exports none :eval yes
  ...
  dbSendQuery(ch,
  #            cat(
    sprintf("update bibliometric set title = '%s', year_published = %s where id = %s",  gsub("'", '"', titl), ct[[1]]$year, id)
              )
  }
#+end_src
...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  dbSendQuery(ch,
  #            cat(
    sprintf("update bibliometric set title = '%s', year_published = %s where id = %s",  titl, ct[[1]]$year, id)
              )
  ...
  dbSendQuery(ch,
  #            cat(
  sprintf("insert into approval(bibliometric_id, name) values (%s, '%s')\n", id, name_i)
              )
