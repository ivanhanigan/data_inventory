...
#+begin_src R :session *R* :tangle data_inventory.R :exports none :eval no
  ...
  ## "studyextent", "temporalcoverage_daterange", "methods_protocol", "sampling_desc", "method_steps", 
    qc <- as.data.frame(t(datasets[j,c("shortname", "title", "creator", "contact_email", "abstract", "additional_metadata", "alternate_identifier", "access_rules", "recommended_citation", "licence_code", "accessibility")]))
    names(qc) <- c("metadata_field")
    qc[,1] <- gsub("\n", " | ", qc[,1])
  if(
