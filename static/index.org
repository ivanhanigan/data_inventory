...
cd ~/postgres_backups
gunzip < data_inventory_hanigan_dev4_2016-11-06-1054.gz | psql -U postgres data_inventory_hanigan_dev4_backup
...
#+begin_src R :session *R* :tangle no :exports none :eval yes
  ...
  setwd("~/tools/web2py/applications/data_inventory/static/")
  dir()
  prj <- "Project_Exposures"
  dsets <- c("SEEF_exposures")
...
#+begin_src R :session *R* :tangle data_inventory.R :exports none :eval no
  ...
  out_bookfile <-   sprintf("data_inventory_book/%s-%s.Rmd", ifelse(i < 10, sprintf("0%s", i), i), 
    gsub(" ", "-", prj)
  )
  sink(
  out_bookfile
  ...
  datasets <- dbGetQuery(ch,
  sprintf("select t1.*, t3.licence_code
  from dataset t1
  join project t2
  on t1.project_id = t2.id
  ...
  ## "studyextent", "temporalcoverage_daterange", "methods_protocol", "sampling_desc", "method_steps", 
    qc <- as.data.frame(t(datasets[j,c("shortname", "title", "creator", "contact_email", "abstract", "additional_metadata", "access_rules", "recommended_citation", "licence_code")]))
    names(qc) <- c("metadata_field")
    qc[,1] <- gsub("\n", " | ", qc[,1])
  if(
