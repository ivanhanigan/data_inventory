...
- removed           Field('title','string',requires=IS_NOT_EMPTY())).process()
...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  ch <- connect2postgres2("data_inventory_ltern_dev_3")
...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  dbWriteTable(ch, "thesaurus_ltern_insert",
               ky, append = F, row.names=F)
...
#+begin_src R :session *R* :tangle no :exports none :eval yes
  cat(sprintf('
  pg_dump -h localhost -p 5432 -U postgres -F t -v -i -f "/home/ivan_hanigan/Dropbox/data/data_inventory_hanigan/backups/backup_file-%s.backup" -n "public" data_inventory_hanigan_dev
  ',gsub(":","-",gsub(" ","-",Sys.time()))))
...
#+begin_src R :session *R* :tangle data_deposit_form.Rmd :exports none :eval no :padline no
  ...
  dat <- sqldf(
  sprintf("select t1.title as project_id, 
  t1.title as project_title, 
  t1.personnel_data_owner,
  t1.personnel_owner_organisationname,
  t1.funding,
# keywords
  ...
  t2.abstract,
  t2.associated_party,
  t2.geographicdescription,
  t2.boundingcoordinates
  from project t1
  join dataset t2
  ...
  #### dataset help
  help  <- sqldf("select t1.eml_node, t1.help_comment, t1.datinv
    from crosswalk t1
    where eml_node like '%dataset%'
    or eml_node like '%project%'",
    connection = ch)
  head(help)
  ...
  # for(i in 1:nrow(dat)){
    ...
    dat_i <- data.frame(V1 = names(dat), V2=t(dat[i,]))
  dat_i
  ...
  ky <- data.frame(index1=qc3[1,1], index2="", variable = "keywords", value = ky, help="Keywords or phrases that concisely describe the resource. Example is biodiversity. Use a controlled vocabulary thesaurus")
  
  
  qc_out <- rbind(qc3[1:10,], ky)
  qc_out <- rbind(qc_out, qc3[11:nrow(qc3),])
  qc_out[,1:3]
  qc_out
  ...
  sprintf("select 
  t3.entityname,
  t3.entitydescription,
  t3.physical_distribution, t3.entity_methods
  from project t1
  join dataset t2
  on t1.id = t2.project_id
  join entity t3
  ...
  title2 <- paste(c(j, "entity", as.character(ent_j[1,2])),
          collapse = ", ", sep = "")
    title2
    qc_ent <- merge(ent_j, help_ent, by.x = "V1", by.y = "datinv")
  ...
  qc_ent3 <- data.frame(index = rep(title2, nrow(qc_ent2)),
                        ...
                        meta = qc_ent2)
  names(qc_ent3) <- names(qc_out)
  qc_out <- rbind(qc_out, qc_ent3)
  qc_out
  
  }
  ```
  ```{r, echo = F, eval = T, results = "asis"}
  # print(kable(qc_out), row.names = F))
  library(xtable)
  print(xtable(qc_out), type = "html", include.rownames = F)
  #write.csv(qc_out, paste(dset, "_data_deposit_form.csv", sep = ""), row.names = F)
  ```
