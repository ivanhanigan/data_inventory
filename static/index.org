...
#+begin_src R :session *R* :tangle no :exports none :eval yes
  ...
  setwd("~/tools/web2py/applications/data_inventory/static/")
  dir()
  prj <- "Air_Pollution_Monitoring_Stations_NSW"
  dsets <- c("OEH_monitor_Sydney_2007_2014")
...
#+begin_src R :session *R* :tangle data_inventory.R :exports none :eval no
  ...
  ## "studyextent", "temporalcoverage_daterange", "methods_protocol", "sampling_desc", "method_steps", 
    qc <- as.data.frame(t(datasets[j,c("shortname", "title", "creator", "contact_email", "abstract", "additional_metadata", "alternate_identifier", "access_rules", "recommended_citation", "licence_code")]))
    names(qc) <- c("metadata_field")
    qc[,1] <- gsub("\n", " | ", qc[,1])
  if(
  ...
  if(server == "coesra") pth  <- file.path("/home/public_share_data/ivan.hanigan_canberra", pth)
