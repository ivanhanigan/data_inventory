...
#+RESULTS: go
: /home/ivan_hanigan/tools/web2py/applications/data_inventory/static/data_inventory_book
...
#+begin_src R :session *R* :tangle no :exports none :eval yes
  ...
  library(rmarkdown)
  library(bookdown)
  table_type = "latex" #"html" # or latex
  show_exemplar <- F
  library(knitr)
  library(disentangle)
  setwd("~/tools/web2py/applications/data_inventory/static/")
  dir()
  prj <- "StreetPro"
  dsets <- c("StreetPro_Navigation_2012")
...
#+RESULTS: go
: TRUE
*** TODO short summary of EML fields for projects/datasets homepage
**** head
  output: html_document
#+begin_src R :session *R* :tangle EML_metadata_short.Rmd :exports none :eval no :padline no
  ...
  ch  <- connect2postgres2("data_inventory_hanigan_dev4")  
  ##ch <- dbConnect(drv, dbname = "~/tools/web2py/applications/data_inventory_to_get/databases/storage.sqlite")
  #prj <- "Air_Pollution_Monitoring_Stations_WA"
  #dset <- "AP_WA_19942012"
...
#+begin_src R :session *R* :tangle EML_metadata_short.Rmd :exports none :eval no :padline no
  ...
  on t2.id = t3.dataset_id
  where t1.title = '%s'
  and t2.shortname = '%s' order by entityname", prj, dset)
  )
...
#+end_src
** TODO dataset metadata SQLITE VERSION FOR CASSANDRA IN GOT
https://cloudstor.aarnet.edu.au/plus/apps/files/?dir=/Shared/CAR_staging_area/data_management_plan/data_inventory_got/static
*** go-code
#+name:go
#+begin_src R :session *R* :tangle /home/ivan_hanigan/tools/web2py/applications/data_inventory_got/static/EML_metadata_short_go.R :exports none :eval yes
  #### name:go ####
  projdir <- "~/tools/web2py/applications/data_inventory_got/static/"
  setwd(projdir)
  library(rmarkdown)
  library(bookdown)
  library(sqldf)
  ...
  if(exists('ch'))   dbDisconnect(ch)  
  drv <- dbDriver("SQLite")
  ch <- dbConnect(drv, dbname = "~/tools/web2py/applications/data_inventory_got/databases/storage.sqlite")
  #### name:summary of project info ####
  table_type = "html" #or "latex"
  show_exemplar <- F # this adds notes like a FAQ

  projs <- dbGetQuery(ch, "select id, title from project")
  projs # choose
  prj <- "NASA_FIRMS_FIRE_AND_SMOKE"
  pid <- projs$id[which(projs$title == prj)]
  dsets <- dbGetQuery(ch,
  sprintf("select title 
  from dataset
...
**** head
  output: html_document
#+begin_src R :session *R* :tangle /home/ivan_hanigan/tools/web2py/applications/data_inventory_got/static/EML_metadata_short.Rmd :exports none :eval no :padline no
  ---
  title: EML metadata fields
  classoption: landscape
  ---
...
#+begin_src R :session *R* :tangle /home/ivan_hanigan/tools/web2py/applications/data_inventory_got/static/EML_metadata_short.Rmd :exports none :eval no :padline no
  ...
  sprintf("select shortname,title,
    ...
    metadataprovider,
    access_rules,
    distribution_methods, publisher
   from dataset
   where shortname ='%s'", dset)
  )
  dat_i <- data.frame(V1 = names(dat), V2=t(dat[1,]))
  ...
  dat <- dbGetQuery(ch,
  sprintf("select t1.id, dataset_id, data_owner, data_owner_contact, licence_code, licence_text, licencee, path_to_licence, special_conditions, accessibility
  from intellectualright t1
  join dataset t2
  on t1.dataset_id =  t2.id
   where shortname ='%s'", dset)
...
#+begin_src R :session *R* :tangle /home/ivan_hanigan/tools/web2py/applications/data_inventory_got/static/EML_metadata_short.Rmd :exports none :eval no :padline no
  # Dataset or study metadata   
  ```{r, echo = F, eval = T, results="asis"}  
  if(!is.na(qc_out_raw[qc_out_raw$variable == "publisher","value"])) qc_out_raw[qc_out_raw$variable == "publisher","value"] <- paste0("[Data set] Retrieved from ", qc_out_raw[qc_out_raw$variable == "publisher","value"])
  if(
     is.null(qc_out_raw[which(qc_out_raw$variable == "recommended_citation"),"value"]) |
     is.na(qc_out_raw[which(qc_out_raw$variable == "recommended_citation"),"value"]) |
     qc_out_raw[which(qc_out_raw$variable == "recommended_citation"),"value"] == ""
     )  qc_out_raw[which(qc_out_raw$variable == "recommended_citation"),"value"]  <-  paste(sapply(c(qc_out_raw[which(qc_out_raw$variable == "creator"),"value"], qc_out_raw[which(qc_out_raw$variable == "pubdate"),"value"], qc_out_raw[which(qc_out_raw$variable == "title"),"value"], qc_out_raw[which(qc_out_raw$variable == "publisher"),"value"]), as.character), sep = "", collapse = ", ")

  qc_out <- qc_out_raw[qc_out_raw$variable %in% c('recommended_citation',
  'additional_metadata', 'alternate_identifier',
  'studyextent',
  'temporalcoverage_daterange',
  'methods_protocol',
  'sampling_desc',
  ...
  'data_owner',
  'data_owner_contact',
  'special_conditions', 'accessibility'
  ),]
  qc_out <- rbind(qc2, qc_out)
  if(
  qc_out[qc_out$variable == "licence_code",'value'] == ""
...
#+end_src
** checkout from master to child SQLITE, assumes you want to replace child (and manually removed it) TODO int rights accessibility missing
shared with cassandra on cloudstor
~/Desktop/data_inventory_query_ETL.R
#+begin_src R :session *R* :tangle no :exports none :eval no
  require(swishdbtools)
  library(RSQLite)
  drv <- dbDriver("SQLite")
  ...
  ch_source <- connect2postgres2("data_inventory_hanigan_dev4")  
  ##ch_source <- dbConnect(drv, dbname = "~/Downloads/storage.sqlite")
  ##ch_target <- dbConnect(drv, dbname = "~/tools/web2py/applications/data_inventory_to_get/databases/storage.sqlite")
  ##ch_target <- dbConnect(drv, dbname = "~/tools/web2py/applications/data_inventory_got/databases/storage.sqlite")
  ...
  ##projs <- c('Satellite_NO2_LUR','Satellite_PM25_LUR')
  collection <- "related_project = 'Environment general' or related_project = 'Environment general AND ResearchData' or related_project = 'ResearchProjects_CAR'"
  projs <- dbGetQuery(ch_source, sprintf("select title, project_abstract, personnel_data_owner
    from project where %s
    order by title", collection))
  print(projs$title)
  ...
  for(i in 1:nrow(projs)){
      ...
      prj <- projs[i,1]
      print(prj)
      datasets <- dbGetQuery(ch_source,
                             sprintf("select t2.title as proj_title, t1.*, t3.licence_code, t3.accessibility
      from dataset t1
      join project t2
      on t1.project_id = t2.id
      left join (select * from intellectualright) t3
      ...
      if(i == 1){
          ...
          projs_todo <- rbind(projs_todo, datasets)
      }
  }    
  projs_todo[,1:3]
  names(projs_todo)
  projs_todoV2 <- unique(projs_todo[,1])
  projs_todoV2
  for(proj in projs_todoV2[-1]){
  ## get the project id
  ##proj <- "ABS_data"
  ## proj <- projs_todoV2[1]
  proj    
  ...
  # get datasets
  dsets  <- dbGetQuery(ch_source,
                       paste("select * from dataset where provision_status = 'Published' and project_id = ",pid, sep = "")
                       )
  dsets[,1:3]
