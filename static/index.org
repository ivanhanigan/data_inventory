...
** TODO find home for distribution 
Distribution strategy, including:
- for paper likely journal, conference presentations and other communication vehicles
- for data
- for code

This deals with IP and copyright, so maybe in intellectual right module?
...
#+end_src
*** COMMENT dataset methods/studyextent
...
#+end_src
*** TODO methods sampling, steps, protocol
**** EML
...
**** Include: 
# methods protocol
Study type: give a list like time-series, cross-sectional, spatial, exploratory, confirmatory
...
# method steps
Data cleaning strategy, and process for handling missing data
...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  ## dbSendQuery(ch, "delete from crosswalk2")
...
#+end_src
** summary of EML fields for projects 
*** head
#+begin_src R :session *R* :tangle EML_metadata_fields.Rmd :exports none :eval no :padline no
  ...
  #ch  <- connect2postgres2("data_inventory_hanigan_dev4")
  #prj <- "Climatic and Agricultural Drought"
  #dset <- "DROUGHT-BOM-GRIDS"
...
* deploy to gislibrary
scp -i ~/tools/macpro_ssh/.ssh/id_rsa ivan@gislibrary.anu.edu.au:/var/www/html/web2py/applications/data_inventory_air_pollution/models/db.py ~/Desktop/
use ediff buffers then reload


# TODO  this did not work for some unkown reason.  give up and do mods manually

scp -i ~/tools/macpro_ssh/.ssh/id_rsa  db.py ivan@gislibrary.anu.edu.au:/home/ivan 

log in and move it to the spot as sudo 
 ssh -i ~/tools/macpro_ssh/.ssh/id_rsa ivan@gislibrary.anu.edu.au
[ivan@gislibrary ~]$ sudo su
mv /var/www/html/web2py/applications/data_inventory_air_pollution/models/db.py db_py_backup_20151018.txt
[root@gislibrary ivan]# 
mv db.py /var/www/html/web2py/applications/data_inventory_air_pollution/models/
chown w2p_user /var/www/html/web2py/applications/data_inventory_air_pollution/models/db.py
